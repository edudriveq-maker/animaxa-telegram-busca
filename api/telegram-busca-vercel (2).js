...
async function listarColecao(colecao) {
  try {
    ...
    console.error(`[telegram-busca] Firestore ao vivo falhou pra "${colecao}": ${errFirestore.message}`);
    try {
      return await listarColecaoViaBackup(colecao);
    } catch (errBackup) {
      throw new Error(`Firestore: ${errFirestore.message} | Backup: ${errBackup.message}`);
    }
  }
}
