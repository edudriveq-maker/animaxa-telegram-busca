...
async function listarColecaoCompleta(colecao) {
  ...
  for (;;) {
    ...
    url.searchParams.set("pageSize", String(PAGINA_FIRESTORE));
    if (pageToken) url.searchParams.set("pageToken", pageToken);

    const res = await fetch(url);
    if (!res.ok) {
      throw new Error(`Firestore (listDocuments) respondeu ${res.status} para "${colecao}"`);
    }
...
async function listarColecao(colecao) {
  try {
    ...
    console.error(`[telegram-busca] Firestore ao vivo falhou pra "${colecao}", tentando backup: ${err.message}`);
    return listarColecaoViaBackup(colecao);
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await chamarTelegram("sendMessage", {
      chat_id: chatId,
      text: "⚠️ Não consegui consultar o catálogo agora. Tenta de novo em instantes.",
    });
  }
